...
// ============================================================================
ClientId MonitoredObject::register_client (yat::uint32 _pp,
                                           const std::string& _iop)
{
  ...
  //- force next export for new client
  this->force_next_export_ = true;
...
// ============================================================================
bool MonitoredObject::has_registered_clients ()
{
...
//=============================================================================
int MonitoredObject::change_polling_period (yat::uint32 _pp, ClientId _c)
{
  if (_c == -1)
  {
    ClientsMapIt it = this->clients_.begin();
...
void MonitoredAttribute::export_value ()
{
  ...
  //- get dev desc
  DevDescriptor * ddesc = DEV_REP->device_desc(this->tango_dev_name_, false);
  if (ddesc == 0) 
  {
  ...
  for (; it != end; ++it)
  {
    bool force_export = this->force_next_export_;
    bool not_suspended = ! it->second.suspended;
    bool its_time_update_value = it->second.export_timer_.elapsed_msec() >= (0.9 * it->second.pp);

    if (force_export || (not_suspended && its_time_update_value))
    {
#if defined (_VERBOSE_)
  		std::cout << "Tango-Binding::exporting monitor value for "
                ...
                << " to "
                << it->second.path
                << (force_export ? " (forcing export)" :  "")
                << std::endl;  
			yat::Timer t;
#endif
      //- reset <force_next_export> flag
      this->force_next_export_ = false;
...
// ============================================================================
void MonitoredDevice::import_values (yat::uint32 ms_to_next_import)
{
#if defined(_PER_ATTR_POLLING_PERIOD_)
  ...
  for (; cur != end; ++cur) 
  {
      ...
      al.push_back(cur->first);
  }
#else
  //- we need this later
  MonitoredAttributeMapIt cur;
  //- attribute list to read
  std::vector<std::string>& al = this->attr_list_; 
#endif
  ...
  else
  {
    ...
    try
    {
      ...
      values = ddesc->proxy()->read_attributes(al);
      for (size_t a = 0; values && (a < al.size()); a++)
      {
        cur = this->mattr_map_.find(al[a]);
    ...
    {
      XOP_STDOUT("Tango-Binding::MonitoredDevice::import_values::Tango exception caught");
      XOP_STDOUT(df);
      for (size_t a = 0; a < al.size(); a++)
      {
        cur = this->mattr_map_.find(al[a]);
    ...
    {
      XOP_STDOUT("Tango-Binding::MonitoredDevice::import_values::unknown exception caught");
      for (size_t a = 0; a < al.size(); a++)
      {
        cur = this->mattr_map_.find(al[a]);
        cur->second->attach_error();
