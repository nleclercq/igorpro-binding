...
int DevDescriptor::update_attributes_datafolder (const std::string& _argout)
{
  ...
  std::string attr_alias("--");
    ...
    do 
    {
      ...
      if(XDK_UTILS->create_df_obj(attr_df, "access", (yat::int32)ail[i].writable)) 
      {
        result = kError; 
        break;
      }
      if(XDK_UTILS->create_df_obj(attr_df, "format", (yat::int32)ail[i].data_format)) 
      {
        result = kError; 
        break;
      }
      if(XDK_UTILS->create_df_obj(attr_df, "max_dim_x", (yat::int32)ail[i].max_dim_x))
      {
        result = kError; 
        break;
      }
      if(XDK_UTILS->create_df_obj(attr_df, "max_dim_y", (yat::int32)ail[i].max_dim_y)) 
      {
        result = kError; 
        break;
      }
...
//=============================================================================
// DevRepository::reload_device_interface
//=============================================================================
int DevRepository::reload_device_interface (const std::string& _device_name)
{
  //- enter critical section
  yat::AutoMutex<yat::Mutex> guard(this->lock_);
  ...
  if ( it == this->dev_map_.end() ) 
  {
    DevDescriptor* d = this->device_desc(_device_name, true);
    return d ? kNoError : kError;
  }

  //- search failed, create the descriptor
  DevDescriptor* ddesc = it->second;
  if (ddesc == 0) 
  {
    XDK_UTILS->set_error("XOP internal error",
                         "unexpected null pointer to DevDescriptor!",
                         "DevRepository::reload_device_interface");
    return kError;
  }
