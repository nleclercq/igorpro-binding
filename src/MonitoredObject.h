...
protected:
  ...
  //- register client 
  ClientId register_client (yat::uint32  pp, 
  ...
  int change_polling_period (yat::uint32 pp, ClientId c = -1);

  //- time to pollling period expiration
  yat::uint32 num_ms_to_polling_period_expiration ();
...
protected:
  ...
  //- import values
  void import_values (yat::uint32 ms_to_next_import);
  ...
  yat::uint32 minimum_polling_period ();

  //- suspends a monitor 
  int suspend_monitor (const std::string& obj_name, yat::int32 client_id = -1);

  //- resumes a monitor 
  int resume_monitor (const std::string& obj_name, yat::int32 client_id = -1);
