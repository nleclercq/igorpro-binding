...
public:
  ...
  yat4tango::ThreadSafeDeviceProxy * proxy () const;
  // Returns the device proxy.

  const Tango::CommandInfoList& cmd_list () const;
  // Returns the device commands list.

  const Tango::AttributeInfoList& attr_list () const;
  // Returns the device attributes list

  DataFolderHandle data_folder () const;
  ...
  int update_commands_datafolder (const std::string& argout);
  //- Stored commands info into the dedicated Igor datafolder.

  int update_attributes_datafolder (const std::string& argout);
  //- Stores attributes info the dedicated Igor datafolder.

private:
  DevDescriptor (std::string _dev_name);
  // Ctor

  void update_attr_list_i () 
    throw (Tango::DevFailed);
  // Update the device attributes list

  int attr_exists_i (const std::string& _attr_name, bool _refresh_local_list);
  // Returns the attr. identifier or -1 if no such attr.
...
public:
  static DevRepository* instance ();
  // Returns the unique instance of <DevRepository>.
    
  DevDescriptor* device_desc (const char* device_name, bool connect = true);
  // Providing a device name, returns its "device proxy" or NULL on error.

  DevDescriptor* device_desc (const std::string& device_name, bool connect = true);
  // Providing a device name, returns its "device proxy" or NULL on error.

  int reload_device_interface (const std::string& device_name);
  //- Reload the specified device interface
