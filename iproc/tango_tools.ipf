...
//==============================================================================
// fonction : tools_kill_wave
//------------------------------------------------------------------------------
// kill the specified wave
//==============================================================================
function tools_kill_wave (wn)
	String wn
	Wave wv = $wn
	if (WaveExists(wv))
		KillWaves/Z wv
...
//==============================================================================
// fonction : tools_kill_waves
//------------------------------------------------------------------------------
// kill the specified waves
//==============================================================================
function tools_kill_waves (wl)
	String wl
	KillWaves/Z $wl
end		
//==============================================================================
...
function tools_df_save_to_file (df, [path])
	...
	tools_df_write_objects_to_file(df, refNum)
	Close refNum
	return kNO_ERROR
end
//==============================================================================
// static fonction : tools_df_write_objects_to_file
//==============================================================================
static function tools_df_write_objects_to_file (df, refNum)
	String df
	Variable refNum
	String str
	if (strlen(df) == 0 || DataFolderExists(df) == 0)
	...
	do
		...
		tools_df_write_objects_to_file(next_df, refNum)
		index += 1
	while(1)
	String obj
		...
		strswitch (type)
			...
			case kSWAV:
				...
				if (! cmpstr(type, kSWAV))
					...
					Make/O/D/N=(dim0,dim1,dim2,dim3) $obj_path 
				endif
				Variable i
...
//==============================================================================
// tools_trunc_obj_name
//==============================================================================
function tools_trunc_obj_name (p, s)
	String &p
	String s;
	Variable l = strlen(p) + strlen(s)
	if ( l > 31 )
		Variable i = 1
		String tmp = p
	  	do 
