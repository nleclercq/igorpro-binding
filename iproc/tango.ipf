	...
	switch (argout_type)
		...
		case kDEVSTRING:
			if (strlen(arg_out.val_path))
			   ...
			   argout_str = LowerStr(cmd)
				tools_trunc_obj_name(argout_str, "_out")
		...
		case kDEVVARDOUBLESTRINGARRAY:
			if (strlen(arg_out.num_wave_path) && strlen(arg_out.str_wave_path))
				...
				String n_argout_str = LowerStr(cmd)
			   tools_trunc_obj_name(n_argout_str, "_nout")
				String s_argout_str = LowerStr(cmd)
			   tools_trunc_obj_name(s_argout_str, "_sout")
				argout_str = n_argout_str + kNAME_SEP + s_argout_str
				arg_out.num_wave_path = GetDataFolder(1) + n_argout_str;
				arg_out.str_wave_path = GetDataFolder(1) + s_argout_str;
			endif
			break
		default:
			txt = "TANGO API ERROR in function 'tango_cmd_inout' - "
...
function tango_read_attr (attr_val_io)
	...
	switch (attr_val_io.format)
		...
		default:
		   break
	endswitch
	if (attr_val_io.format == kIMAGE)
		wave w = $attr_val_io.val_path
		if (WaveExists(w))
	      MatrixTranspose w
		endif
	endif
	...
	for (n = 0; n < attr_vals_io.nattrs; n += 1)
		...
		switch (attr_vals_io.vals[n].format)
			...
			case kIMAGE:
				...
				break
		endswitch
		if (attr_vals_io.vals[n].format == kIMAGE)
	   		wave w = $attr_vals_io.vals[n].val_path
	   		if (WaveExists(w))
	   			MatrixTranspose w
	   		endif
	   	endif
...
function tango_read_attrs_list (dev_name, user_attr_list)
	...
	for (n = 0; n < DimSize(user_attr_list, 0); n += 1)
		...
		if (WaveExists($result_name) && WaveDims($result_name) == 2)
			MatrixTranspose $result_name
		endif	
		KillVariables /Z $(result_name + "_ts")
	endfor
	return kNO_ERROR
...
function tango_write_attr (attr_val_io)
	...
	switch (format)
		...
		case kSPECTRUM:
			...
			break
		case kIMAGE:
			string cpy_name = attr_val_io.val_path + "_t"
		  	wave w = $attr_val_io.val_path
		   if (WaveExists(w))
		   		Duplicate /O w, $cpy_name
		   		MatrixTranspose $cpy_name
		   	endif
			if (tango_write_attribute(attr_val_io.dev, attr_val_io.attr, cpy_name) == kERROR)
				tools_kill_wave(cpy_name)
				return kERROR
			endif
			tools_kill_wave(cpy_name)
			break
...
function tango_write_attrs (attr_vals_io)
	...
	String tmp_wave_list = "", attr_list = "", val_name = ""
	...
	for (n = 0; n < attr_vals_io.nattrs; n += 1)
		...
		switch (attr_format)
		   ...
		   case kSPECTRUM:
				...
				break
		   case kIMAGE:
		   		string cpy_name = GetDataFolder(1) + "transposed_cpy_" + num2str(n)
		  		wave w = $attr_vals_io.vals[n].val_path
		   		if (WaveExists(w))
		   			Duplicate /O w, $cpy_name		   			
		   			tmp_wave_list = tools_str_list_add(cpy_name, tmp_wave_list)
		   			MatrixTranspose $cpy_name
		   		endif
				attr_list = tango_attr_val_list_add(attr_list, attr_vals_io.vals[n].attr, cpy_name)
				break
			default:
				break
		endswitch
	...
	if (tango_write_attributes(attr_vals_io.dev, attr_list) == kERROR) 
		tools_kill_waves(tmp_wave_list)
		tango_leave_tmp_df(cur_df)
		return kERROR
	endif
	tools_kill_waves(tmp_wave_list)
...
function tango_enter_device_df (device_name, [prev_df])
	...
	if (! DataFolderExists(device_df))
		if (tango_reload_dev_interface(device_name) == kERROR)
		   tango_print_error()
			return kERROR
		endif
	endif
...
function tango_enter_attrs_df  (dev, [prev_df])
	...
	if (! DataFolderExists(attrs_df))
		if (tango_reload_dev_interface(dev) == kERROR)
		   tango_print_error()
			return kERROR
		endif
	endif
...
function tango_monitor_stop_list (dev, attrs, cids)
	...
	Wave/T attrs
	Wave cids
	//Variable n = numpnts(attrs) voir avec Nicolas
	Variable n = dimsize(attrs,0)
	Variable i = 0
	Variable err = kNO_ERROR
