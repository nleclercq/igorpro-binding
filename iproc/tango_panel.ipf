...
function tp_show_error ( )
	...
	ListBox err_list,win=tango_panel,widths={ww[0],ww[1],ww[2],ww[3]}
...
function tp_ctrlproc_update_list_but (ctrlName) : ButtonControl
	...
	Variable min_w = FontSizeStringWidth("Courier New", kLB_FONTSIZE, 0, "Device")
	tools_get_listbox_colwidths(dlist, ww, min_w, f="Courier New", fs=kLB_FONTSIZE)
			...
			if (lba.eventMod & 0x10)
				...
				switch (V_Flag)
					...
					case 5:
						...
						break
					case 7:
						tango_reload_device_interface(lba.listWave[lba.row][1])
						lba.eventMod = 0
...
function tp_update_attr_list (device_name)
	...
	Make /O /N=(17) ww
	Variable min_w = FontSizeStringWidth("Courier New", kLB_FONTSIZE, 0, "WAttrName")
	tools_get_listbox_colwidths(alist, ww, min_w, f="Courier New", fs=kLB_FONTSIZE)
...
function tp_update_prop_list (device_name)
	...
	if (num_prop)
		...
		Make/O/N=2 ww
		Variable min_wd = FontSizeStringWidth("Courier New", kLB_FONTSIZE, 0, "Value")
		tools_get_listbox_colwidths(plist, ww, min_wd, f="Courier New", fs=kLB_FONTSIZE)
