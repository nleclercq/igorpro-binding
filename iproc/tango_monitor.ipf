...
static constant kDEFAULT_PP = 1.0 
//------------------------------------------------------------------------------
...
static strconstant kUSER_CB     = "UCB"
static strconstant kUSER_CB_DEP = "UCBDEP"
static strconstant kSMIN        = "SMIN"
static strconstant kSMAX        = "SMAX"
static strconstant kXUNIT       = "XUNIT"
static strconstant kYUNIT       = "YUNIT"
//------------------------------------------------------------------------------
static constant kTAB_PROJ    = 0
static constant kTAB_COLORS  = 1
static constant kTAB_MONCTRL = 2
static constant kTAB_PGRID   = 3
//------------------------------------------------------------------------------
...
// tmon_create
//==============================================================================
function tmon_create (dev, attr, [user_cb])
	String dev
	String attr
	String user_cb
	...
	wns.smin = wns.val_path + "_smin"
	wns.smax = wns.val_path + "_smax"
	wns.xunit = wns.val_path + "_xunit"
	wns.yunit = wns.val_path + "_yunit"
	Variable/G $wns.dep = 0
	if (! ParamIsDefault(user_cb))
		wns.cb = user_cb
		wns.cb_dep = wns.val_path + "_cb"
	   Variable/G $wns.cb_dep = 0
	else
		wns.cb = ""
		wns.cb_dep = ""
	endif
	...
	switch (wns.format) 
		...
		case kSPECTRUM:
			...
			SetFormula $wns.dep, formula_str
			Variable/G $wns.smin = 0
			Variable/G $wns.smax = 100
			String/G $wns.xunit = "uu"
			String/G $wns.yunit = "uu"
			SetVariable min, win=$wns.win, value=$wns.smin
			SetVariable max, win=$wns.win, value=$wns.smax
			SetVariable xunit, win=$wns.win, value=$wns.xunit
			SetVariable yunit, win=$wns.win, value=$wns.yunit
			if (has_state_attr)
		...
		case kIMAGE:
			...
			formula_str += "(" + wns.val_path + ",\"" + wns.val_path + "\",\"" + wns.win + "\")"
			SetFormula $wns.dep, formula_str
			Variable/G $wns.smin = -50
			Variable/G $wns.smax = 50
		   String/G $wns.xunit = "mm"
			String/G $wns.yunit = "mm"
			SetVariable min, win=$wns.win, value=$wns.smin
			SetVariable max, win=$wns.win, value=$wns.smax
			//SetVariable xunit, win=$wns.win, value=$wns.xunit
			//SetVariable yunit, win=$wns.win, value=$wns.yunit
			PopupMenu scale_menu, win=$wns.win, popvalue="50x50"
			if (! ParamIsDefault(user_cb))
				formula_str = user_cb 
				formula_str += "(" + wns.val_path + ",\"" + wns.val_path + "\",\"" + wns.win + "\")"
				SetFormula $wns.cb_dep, formula_str
			endif
			if (has_state_attr)
...
static function tmon_kill (win_name, [kill_win])
	...
	if (strlen(wns.cb_dep))
		dep_var_name = tools_full_obj_path_to_obj_name(wns.cb_dep)
		NVAR/Z cb_dep_var = $dep_var_name
		if (NVAR_Exists(cb_dep_var))
			SetFormula cb_dep_var, ""
			KillVariables/Z $dep_var_name
		endif
	endif
	KillVariables /A
...
//==============================================================================
// function : tmon_spectrum_tab_proc
//==============================================================================
function tmon_spectrum_tab_proc (tca)
	Struct WMTabControlAction &tca
	switch (tca.tab)
		case 0:
			SetVariable max, win=$tca.win, disable=1
			SetVariable min, win=$tca.win, disable=1
			SetVariable xunit, win=$tca.win, disable=1
			SetVariable yunit, win=$tca.win, disable=1
			Button setscale_but, win=$tca.win, disable=1						
			Button pause_but, win=$tca.win, disable=0
			Button kill_but, win=$tca.win, disable=0
			SetVariable pp, win=$tca.win, disable=0
		...
		case 1:
			...
			SetVariable max, win=$tca.win, disable=0
			SetVariable min, win=$tca.win, disable=0
			SetVariable xunit, win=$tca.win, disable=0
			SetVariable yunit, win=$tca.win, disable=0
			Button setscale_but, win=$tca.win, disable=0
			break
	endswitch
end
//==============================================================================
...
function tmon_image_tab_proc (tca)
	...
	switch (tca.tab)
		case kTAB_PROJ:
			...
			CheckBox attach_profile_cb, win=$tca.win, disable=0
			CheckBox pgrid_cb, win=$tca.win, disable=1
			SetVariable max, win=$tca.win, disable=1
			SetVariable min, win=$tca.win, disable=1
			PopupMenu scale_menu,win=$tca.win,disable=1
			Button setscale_but, win=$tca.win, disable=1
			break
		case kTAB_COLORS:
			Button pause_but, win=$tca.win, disable=1
		...
		case kTAB_PGRID:
			...
			CheckBox profile_cb, win=$tca.win, disable=1
			CheckBox attach_profile_cb, win=$tca.win, disable=1
			CheckBox pgrid_cb, win=$tca.win, disable=0
			SetVariable max, win=$tca.win, disable=0
			SetVariable min, win=$tca.win, disable=0
			PopupMenu scale_menu,win=$tca.win,disable=0
			Button setscale_but, win=$tca.win, disable=0
			break
	endswitch
end
...
static function tmon_display_scalar (wns, win_title)
	...
	ValDisplay current_value,win=$wns.win,labelBack=(0,65280,0),frame=2,mode=0
...
static function tmon_display_spectrum (wns, win_title)
	...
	TabControl tab_ctrl,win=$wns.win,tabLabel(1)="Scale"
	TabControl tab_ctrl,win=$wns.win,value=0,disable=0
	TabControl tab_ctrl,win=$wns.win,proc=tmon_spectrum_tab_proc
	...
	Button kill_but,win=$wns.win,font=$kLB_FONT,fSize=kLB_FONTSIZE
	Button kill_but,win=$wns.win,proc=tmon_but_proc_kill
	Button snapshot_but,win=$wns.win,pos={251,24},size={65,20},title="Snapshot"
	Button snapshot_but,win=$wns.win,font=$kLB_FONT,fSize=kLB_FONTSIZE
	Button snapshot_but,win=$wns.win,proc=tmon_but_proc_snapshot
	SetVariable pp,win=$wns.win,pos={121,26},size={116,16}
	SetVariable pp,win=$wns.win,font=$kLB_FONT,fSize=kLB_FONTSIZE
	SetVariable pp,win=$wns.win,title="Update every",format="%g s"
	SetVariable pp,win=$wns.win,limits={kMIN_PP, kMAX_PP, kSTEP_PP}
	SetVariable pp,win=$wns.win, proc=tmon_setvar_proc_pp
	SetVariable max,win=$wns.win,pos={110,27},size={100,16},title="max",disable=1
	SetVariable max,win=$wns.win,font=$kLB_FONT,fSize=kLB_FONTSIZE,format="%g"
	SetVariable min,win=$wns.win,pos={6,27},size={100,16},title="min",disable=1
	SetVariable min,win=$wns.win,font=$kLB_FONT,fSize=kLB_FONTSIZE,format="%g"
	SetVariable xunit,win=$wns.win,pos={217,27},size={70,16},title="x-unit",disable=1
	SetVariable xunit,win=$wns.win,font=$kLB_FONT,fSize=kLB_FONTSIZE,limits={0,0,0}
	SetVariable yunit,win=$wns.win,pos={293,27},size={70,16},title="y-unit",disable=1
	SetVariable yunit,win=$wns.win,font=$kLB_FONT,fSize=kLB_FONTSIZE,limits={0,0,0}
	Button setscale_but,win=$wns.win,pos={377,24},size={86,20},title="Set Scale"
	Button setscale_but,win=$wns.win,font=$kLB_FONT,fSize=kLB_FONTSIZE,disable=1
	Button setscale_but,win=$wns.win,proc=tmon_but_proc_setscale
...
static function tmon_display_image (win_name, win_title, img, img_path)
	...
	TabControl tab_ctrl,win=$win_name,tabLabel(kTAB_PGRID)="Polar Grid."
	...
	CheckBox pgrid_cb,win=$win_name,pos={10,28},size={51,14}
	CheckBox pgrid_cb,win=$win_name,font=$kLB_FONT,fSize=kLB_FONTSIZE
	CheckBox pgrid_cb,win=$win_name,title="On/Off",value=0 
	CheckBox pgrid_cb,win=$win_name,proc=tmon_cb_proc_pgrid,disable=1
	Button setscale_but,win=$win_name,pos={332,24},size={65,20},proc=tmon_but_proc_setscale,title="Set Scale"
	Button setscale_but,win=$win_name,font=$kLB_FONT,fSize=kLB_FONTSIZE,disable=1
	SetVariable max,win=$win_name,pos={153,27},size={80,16},title="max",disable=1
	SetVariable max,win=$win_name,font=$kLB_FONT,fSize=kLB_FONTSIZE,format="%g mm"
	SetVariable min,win=$win_name,pos={68,27},size={80,16},title="min",disable=1
	SetVariable min,win=$win_name,font=$kLB_FONT,fSize=kLB_FONTSIZE,format="%g mm"
	PopupMenu scale_menu,win=$win_name,pos={238,24},size={88,21},proc=tmon_menu_proc_scale,title="scale"
	PopupMenu scale_menu,win=$win_name,value=#"\"10x10;20x20;30x30;40x40;50x50;60x60;70x70;80x80;90x90;100x100\""
	PopupMenu scale_menu,win=$win_name,mode=1,popvalue="50x50",disable=1
	//- DoUpdate
end
//==============================================================================
...
//==============================================================================
// function : tmon_cb_proc_pgrid
//==============================================================================
function tmon_cb_proc_pgrid (cba)
	STRUCT WMCheckboxAction &cba
	if (cba.eventCode != 2)
		return kNO_ERROR
	endif
	...
	tmon_get_win_note(cba.win, wns)
	if ( cba.checked )
		tango_dp_polar_grid_show(cba.win)
	else
		tango_dp_polar_grid_hide(cba.win)
	endif
end
//==============================================================================
...
//==============================================================================
function tmon_open_profiles (win_name, [open_fp])
	String win_name
	Variable open_fp
	if (ParamIsDefault(open_fp))
		open_fp = 0
	endif
	Struct TMonWinNote wns
	tmon_get_win_note(win_name, wns)
	...
	Make/O/D/N=(DimSize($wns.val_path,0)) $wns.xp
	SetScale /P x, DimOffset($wns.val_path,0), DimDelta($wns.val_path,0), "mm", $wns.xp
	wns.xp_dep = wns.xp + "_dv"
	Variable/G $wns.xp_dep
	wns.yp = wns.val_path + "_yp"
	Make/O/D/N=(DimSize($wns.val_path,1)) $wns.yp
	SetScale /P x, DimOffset($wns.val_path,0), DimDelta($wns.val_path,0), "mm", $wns.yp
...
function tmon_xprofile (src, dest_path, iwin, uv)
	...
	if (! strlen(CsrWave(B, iwin)))
		csrB_x = DimSize(src, 0) - 1
		csrB_y = DimSize(src, 1) - 1
	else
		csrB_x = pcsr(B, iwin)
		csrB_y = qcsr(B, iwin)
	endif
	...
	if (tango_compute_image_proj(src, dest, 0, min_x, min_y, max_x, max_y) == -1)
		tango_print_error()
		return kERROR
	endif 
	variable xb =  DimOffset(src,0) + csrA_x * DimDelta(src,0)
	variable xe =  DimOffset(src,0) + csrB_x * DimDelta(src,0)
	SetScale /I x, xb, xe, "mm", dest
	return kNO_ERROR
end
//==============================================================================
...
function tmon_yprofile (src, dest_path, iwin, uv)
	...
	variable xb =  DimOffset(src,0) + csrA_y * DimDelta(src,0)
	variable xe =  DimOffset(src,0) + csrB_y * DimDelta(src,0)
	SetScale /I x, xb, xe, "mm", dest
	return kNO_ERROR
end
//==============================================================================
...
function tmon_fprofile (src, dest_path, iwin, uv)
	...
	variable dox = DimOffset(src, 0)
	variable ddx = DimDelta(src, 0)
	variable doy = DimOffset(src, 1)
	variable ddy = DimDelta(src, 1)
	SetScale /P x, 0, 1, "", src
	SetScale /P y, 0, 1, "", src
	ImageLineProfile /SC srcWave=src, xWave=xtrace, yWave=ytrace, width=1
	SetScale /P x, dox, ddx, "mm", src
	SetScale /P y, doy, ddy, "mm", src
...
//==============================================================================
// function : tmon_but_proc_setscale
//==============================================================================
function tmon_but_proc_setscale (ba)
	Struct WMButtonAction &ba
	if (ba.eventCode != 2)
		return kNO_ERROR
	endif
	...
	String cur_df = GetDataFolder(1) 
   Wave w = $wns.val_path
   ControlInfo /W=$ba.win min
   variable min_val = v_value
   ControlInfo /W=$ba.win max
   variable max_val = v_value
   ControlInfo /W=$ba.win pgrid_cb
   variable pgrid_on = v_value
   if (pgrid_on)
   		tango_dp_polar_grid_hide(ba.win)
   endif
   svar xu = $(wns.xunit)
   Setscale /I x, min_val, max_val, xu, w
   svar yu = $(wns.yunit)
   Setscale /I y, min_val, max_val, yu, w
   	SetAxis /W=$ba.win /A
   if (pgrid_on && wns.type == kIMAGE)
   		tango_dp_polar_grid_show(ba.win)
   endif
	SetDataFolder cur_dF
end
//==============================================================================
...
//==============================================================================
// function : tmon_menu_proc_scale
//==============================================================================
function tmon_menu_proc_scale(pa) : PopupMenuControl
	...
	endswitch
	Struct TMonWinNote wns
	tmon_get_win_note(pa.win, wns)
	nvar/Z smin = $wns.smin
	nvar/Z smax = $wns.smax
	switch (pa.popNum)
	 case 1:
	  smin = -5
	  smax = +5 
...
function tmon_win_hook_profile (whs)
	...
	if (last_evt == 12 && whs.eventCode != 12 && pwns.pattached )
		tmon_attach_profiles(whs.winname)
	endif
	Variable has_xp = strlen(pwns.xp_win) > 0
	Variable has_yp = strlen(pwns.yp_win) > 0
	Variable has_fp = strlen(pwns.fp_win) > 0
	switch (whs.eventCode)
		//- win activated
		case 0:
		  if (has_xp)
		    DoWindow /F $ pwns.xp_win
		  endif
		  if (has_yp)
		    DoWindow /F $ pwns.yp_win
		  endif
		  if (has_fp)
		    DoWindow /F $ pwns.fp_win
		  endif
		  if (has_xp || has_yp || has_fp)
		    DoWindow /F $ pwns.win
		  endif
...
//			ControlInfo /W=$win pgrid_cb
//			if (V_Value)
//				tango_dp_polar_grid_show(win, clear_dl=0)
//			endif
//		endif
		something_drawn = 1
	endif
...
static function tmon_set_win_note (win_name, wns)
	...
	win_note = ReplaceStringByKey(kUSER_CB, win_note, wns.cb)
	win_note = ReplaceStringByKey(kUSER_CB_DEP, win_note, wns.cb_dep)
	win_note = ReplaceStringByKey(kSMIN, win_note, wns.smin)
	win_note = ReplaceStringByKey(kSMAX, win_note, wns.smax)
	win_note = ReplaceStringByKey(kXUNIT, win_note, wns.xunit)
	win_note = ReplaceStringByKey(kYUNIT, win_note, wns.yunit)
...
static function tmon_get_win_note (win_name, wns)
	...
	wns.cb = StringByKey(kUSER_CB, win_note)
	wns.cb_dep = StringByKey(kUSER_CB_DEP, win_note)
	wns.smin = StringByKey(kSMIN, win_note)
	wns.smax = StringByKey(kSMAX, win_note)
	wns.xunit = StringByKey(kXUNIT, win_note)
	wns.yunit = StringByKey(kYUNIT, win_note)
end
//==============================================================================
...
function tmon_start_dev_state_monitor (dev, cid, state_path)
	...
	cid = tango_monitor_start(dev, "State", state_path, 1000)
	SetDataFolder cur_df
	return 0
end
//==============================================================================
...
//==============================================================================
// function : tango_dp_polar_grid_show
//==============================================================================
function tango_dp_polar_grid_show (win_name, [clear_dl])
	String win_name 
	Variable clear_dl
	if (ParamIsDefault(clear_dl))
		clear_dl = 1
	endif 
	GetAxis /W=$win_name /Q bottom
	//- erase then set the target drawing layer
	if (clear_dl)
		SetDrawLayer /K /W=$win_name UserFront
	endif
	SetDrawLayer /W=$win_name UserFront
	//- coord system and transparency
	SetDrawEnv /W=$win_name xcoord=bottom, ycoord=left, fillpat=0, save
	//- end of group 
	SetDrawEnv /W=$win_name gstart
	//- centrer cross
	SetDrawEnv /W=$win_name linefgc=(32767, 32767, 32767), dash=0, save
	DrawLine /W=$win_name -0.3, 0, 0.3, 0
	DrawLine /W=$win_name  0, -0.3, 0, 0.3
	Variable max_radius = 10 * round(V_max) / 10
	Variable radius = max_radius
	Variable step = max_radius / 10
	Variable min_radius
	//- radial part of the grid
	SetDrawEnv /W=$win_name linefgc=(32767, 32767, 32767)
	for (; radius >= step ; radius -= step)
		DrawOval /W=$win_name -radius, radius, radius, -radius
		min_radius = radius
	endfor
	//- angular part of the grid
	Variable t
	Variable/C s_coord
	Variable/C e_coord
	Variable max_angle = 2 * pi
	step = 20 * pi / 180
	//- major ticks
	Variable tick_end = 5 * trunc (V_max / 5) + 0.2
	SetDrawEnv /W=$win_name linefgc=(32767, 32767, 32767), save
	Variable angle = 20 * pi / 180
	for (t=20; angle <= max_angle; angle += step, t += 20)
		s_coord = p2rect(cmplx(min_radius,angle))
		e_coord = p2rect(cmplx(max_radius,angle))
		DrawLine /W=$win_name real(s_coord),imag(s_coord),real(e_coord),imag(e_coord)
	endfor
	//- end of group
	SetDrawEnv /W=$win_name gstop
	KillVariables/Z V_min, V_max
end

//==============================================================================
// function : tango_dp_polar_grid_hide
//==============================================================================
function tango_dp_polar_grid_hide (win_name)
	String win_name
	SetDrawLayer /K /W=$win_name UserFront
end
